...
```sh
mkdir -p build/doom
python3 samo-lib/mbr/make-flash.py build/doom/flash.rom
...
`doom.xpm` is the DOOM wordmark, generated by `make-icon.py` from
`doom-logo.webp`. That source is pure black with an alpha channel and 0.24%
mid-tone, so the reduction is area coverage cut at half with nothing to
tune. The checked-in XPM is what the build uses; regenerating wants Pillow.
