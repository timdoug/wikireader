...
static int read_exact(ZIM_FILE *file, uint64_t offset, void *buffer,
		      size_t length)
{
	if (file_lseek64(file->handle, offset) != FILE_ERROR_OK)
		return -1;
		     ...
		     size_t length)
{
	...
	while (length) {
		...
		if (!page_offset && length >= ZIM_FILE_PAGE_SIZE) {
			...
			if (read_exact(file, offset, output, amount))
				return -1;
		} else {
