...
if ! grep -F "$syscall_marker" "$work/boot.log" >/dev/null || \
   ...
   ! grep -F "$signal_expected" "$work/boot.log" >/dev/null; then
	cat "$work/boot.log" >&2
...
grep -E "C33 Linux: entry|Linux version|Memory:|Calibrating delay loop|C33 UART:|Run /init|binfmt_flat: Load|C33: entered userspace|C33 process test|C33 signal test|HARDWARE PASS|c33 shell:|UART RX reached|pid 1|commands 2" \
	"$work/boot.log"
