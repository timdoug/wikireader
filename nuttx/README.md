...
`CONFIG_INTERPRETERS_MICROPYTHON` downloads v1.29.0 the way Lua and Toybox
are downloaded. The port is `apps/interpreters/micropython`: a
`mpconfigport.h`, a `mphalport.c` of about a hundred lines, and a `main` that
starts the interpreter and hands it a heap.

Four choices in that configuration are the port:

* **setjmp for everything the compiler would otherwise do in assembly.**
  `MICROPY_NLR_SETJMP` for the non-local return an exception unwinds through,
  and `MICROPY_GCREGS_SETJMP` so the collector finds references sitting in
  registers — MicroPython has hand-written versions of both for the
  architectures it knows, and the C33 is not one of them. The native code
  emitter is off for the same reason; the bytecode virtual machine is the
  point here anyway.
* **Single-precision floats and arbitrary-precision integers.** There is no
  floating-point unit, so a double is twice the software for no more truth.
  Integers are the other way round: without `MICROPY_LONGINT_IMPL_MPZ`,
  `2**30` overflows, which is not the Python anyone means.
* **The heap comes from NuttX and goes back.** 256 KB (`HEAPSIZE`) taken with
  `malloc` when the interpreter starts and freed when it leaves, so an
  interpreter nobody runs costs nothing but its text — the same bargain the
  Forth's workspace makes.
* **NuttX's filesystem, mounted where MicroPython keeps its own.** The
  interpreter normally owns its filesystems; here `VfsPosix` is mounted at
  the root, so `open`, `os.listdir`, `os.stat` and `import` all end at a
  NuttX descriptor and a path means what it means at the shell prompt.
  `micropython /sd/thing.py` runs a script off the card.

`nuttx.app` grows 212 KB for all of this, which buys the language whole:
classes, closures, generators, comprehensions, exceptions and tracebacks,
`sys` `os` `time` `math` `gc` `io` `struct` `array` `collections`
`micropython` `uctypes`, and a REPL with history and automatic indentation.
`tools/check.py` on the card exercises the lot; `make test` runs it.

Two things cost real time. The first is that MicroPython interns every string
its sources mention into a table generated by preprocessing those sources
with the target compiler — so the table has to be built with NuttX's own
flags, and it has to be rebuilt whenever the configuration or the source list
changes. It was generated once, in the one-time `context` step, and a stale
table announces itself as a hundred undeclared `MP_QSTR_` constants; it is a
prerequisite of the objects now.

The second is that this is a flat build with one symbol namespace, and NuttX
has a `readline()` of its own. The linker quietly gave MicroPython's line
editor the shell's, which takes a buffer and a length where the caller passed
a string object and a prompt. The interpreter started, printed its banner and
its help line, and then emitted erase-line escapes forever with no prompt —
everything else about it working. `mpconfigport.h` renames the five
`readline` symbols; `test_micropython.py` types an expression at the REPL and
insists on seeing the answer.
